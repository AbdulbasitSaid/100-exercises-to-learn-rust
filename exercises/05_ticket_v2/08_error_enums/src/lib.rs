...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionEmptyError) => {
            Ticket::new(title, "Description not provided".into(), status).unwrap()
        }
        Err(TicketNewError::DescriptionLongError) => {
            Ticket::new(title, "Description not provided".into(), status).unwrap()
        }
        Err(TicketNewError::TitleEmptyError) => {
            panic!("Title cannot be empty")
        }
        Err(TicketNewError::TitleLongError) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            ...
            return Err(TicketNewError::TitleEmptyError);
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleLongError);
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionEmptyError);
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionLongError);
        }
